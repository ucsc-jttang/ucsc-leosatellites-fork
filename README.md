# Code Rundown
In the .ned files, the network is structured with arrays for ground stations and satellites, both modeled as extensions of the StandardHost in OMNeT++. Ground stations and satellites incorporate their respective mobility modules, GroundStationMobility and SatelliteMobility, which calculate distances for links such as ground station-to-satellite and satellite-to-satellite. The network is further organized using two key modules, the LeoIpv4NetworkConfigurator and the LeoChannelConstructor. These modules handle the configuration of links and routing of the satellite network, with the LeoChannelConstructor calling LeoIpv4NetworkConfigurator.

The operational flow of the LeoChannelConstructor is: initialization, ISL interface configuration, groundstation link configuration, ppp interface update, channel update, another ISL update, and finally topology generation and routing table population. The first ISL interface configuration is to attach the PPP modules between satellites together with a delay based on their distance from each other. This delay, and all delays, are calculated using the satellite mobility modules present in each satellite. The groundstation link configuration establishes the groundstation to satellite links and updates the igraph topology representation. All PPP Interfaces are then updated with all of the groundstation links. The channel update then takes these PPP interfaces and adds the propagation delay to the groundstation to satellite PPP interfaces.Once the groundstation links and PPP interfaces are setup, the ISLs are added to the topology and Dijkstra’s is run on every network node to populate a nexthop table.

The .ini configuration files specify settings such as the mobility update interval, determining how frequently satellite positions are updated, and constellation size variables including: number of orbital planes, the number of satellites per plane, satellite altitude, orbital plane inclination, the total number of satellites, the number and locations of ground stations, and application-level parameters.
